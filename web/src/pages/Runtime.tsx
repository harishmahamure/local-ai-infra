...
import { CHAT_CURL, IMAGE_CURL } from "../lib/ui";

export function Runtime() {
  const { status } = useStore();
  const [copied, setCopied] = useState("Copy curl");
  const busyGpu = status?.profile === "comfyui" && status.loadState === "LOADED";
  ...
  return (
    <>
      ...
      <section className="card endpoint">
        ...
        {busyGpu ? (
          <p className="error">ComfyUI is loaded. Chat returns 409 GPU_BUSY until you switch to a chat profile.</p>
        ) : null}
        <pre>{CHAT_CURL}</pre>
