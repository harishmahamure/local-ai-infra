#!/usr/bin/env bash
# Launch ComfyUI from its project .venv (torch 2.14 + cu130, sm_120).
set -euo pipefail
...
if [[ ! -x "$PYTHON" ]]; then
  ...
  echo "On the GPU box: cd ${COMFYUI_HOME} && python3 -m venv .venv && .venv/bin/pip install -r requirements.txt" >&2
  exit 1
fi
